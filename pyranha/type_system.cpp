...
bp::object type_generator::operator()() const
{
    ...
    if (it == et_map.end()) {
        ::PyErr_SetString(PyExc_TypeError,
                          ("the type '" + piranha::type_name<decltype(m_t_idx)>() + "' has not been registered").c_str());
...
{
    return "Type generator for the C++ type '" + piranha::type_name<decltype(m_t_idx)>() + "'";
}
...
static inline std::string v_t_idx_to_str(const std::vector<std::type_index> &v_t_idx)
{
    ...
    for (decltype(v_t_idx.size()) i = 0u; i < v_t_idx.size(); ++i) {
        tv_name += piranha::type_name<decltype(v_t_idx[i])>();
