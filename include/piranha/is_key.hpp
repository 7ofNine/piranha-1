...
class is_key
{
    ...
    // (OK) is_key_is_one_type < addlref_t<const T>>::value;
    //(OK)is_key_is_zero_type<addlref_t<const T>>::value;
    //(fail)std::is_same<detected_t<trim_t, T>, T>::value;
    //(fail) std::is_same<detected_t<trim_identify_t, T>, void>::value;
    //(fail) std::is_same<detected_t<print_tex_t, T>, void> ::value;
    //= std::is_same<detected_t<print_t, T>, void>::value;
    //(fail) std::is_same<detected_t<merge_symbols_t, T>, T>::value;
    //fail) std::is_same<detected_t<is_compatible_t, T>, bool>::value;
    //(fail) is_hashable<T>::value;
    //(fail) is_equality_comparable<addlref_t<const T>>::value;
    // (OK) std::is_constructible<T, const symbol_fset &>::value;
    // (OK) is_container_element<T>::value;
