...
TEST_CASE("memory_aligned_malloc_test")
{
    // Test the common codepath.
    auto ptr = aligned_palloc(0, 0);
    CHECK(ptr == nullptr);
    CHECK_NOTHROW(aligned_pfree(0, ptr));
    ptr = aligned_palloc(123, 0);
    CHECK(ptr == nullptr);
    CHECK_NOTHROW(aligned_pfree(123, ptr));
    ...
    CHECK_THROWS_AS(aligned_palloc(3, 1), std::bad_alloc);
    CHECK_THROWS_AS(aligned_palloc(7, 1), std::bad_alloc);
    ...
    if (sizeof(void *) % alignof(int) == 0 && !(sizeof(void *) & (sizeof(void *) - 1u))) {
        ...
        CHECK(std::equal(v.begin(), v.end(), static_cast<int *>(ptr)));
        // NOTE: no need to call dtor, int has trivial destruction.
        CHECK_NOTHROW(aligned_pfree(sizeof(void *), ptr));
    }
#elif defined(_WIN32)
    ...
    if (16 % alignof(int) == 0) {
        ...
        std::copy(v.begin(), v.end(), static_cast<int *>(ptr));
        CHECK(std::equal(v.begin(), v.end(), static_cast<int *>(ptr)));
        CHECK_NOTHROW(aligned_pfree(16, ptr));
    }
#endif
}

// NOTE: here we are assuming we can do some basic arithmetics on the alignment and size values.
TEST_CASE("memory_alignment_check_test")
{
    CHECK(piranha::alignment_check<int>(0));
    CHECK(piranha::alignment_check<long long>(0));
    CHECK(piranha::alignment_check<std::string>(0));
#if defined(PIRANHA_HAVE_POSIX_MEMALIGN)
    ...
    if (sizeof(void *) >= alignof(int) && !(sizeof(void *) & ((sizeof(void *) - 1u)))) {
        ...
        CHECK(piranha::alignment_check<int>(sizeof(void *) * 2u));
        CHECK(piranha::alignment_check<int>(sizeof(void *) * 4u));
        CHECK(piranha::alignment_check<int>(sizeof(void *) * 8u));
    }
    if (sizeof(void *) >= alignof(long) && !(sizeof(void *) & ((sizeof(void *) - 1u)))) {
        CHECK(piranha::alignment_check<long>(sizeof(void *) * 2u));
        CHECK(piranha::alignment_check<long>(sizeof(void *) * 4u));
        CHECK(piranha::alignment_check<long>(sizeof(void *) * 8u));
    }
#else
    CHECK(piranha::alignment_check<int>(alignof(int)));
    CHECK(piranha::alignment_check<std::string>(alignof(std::string)));
    CHECK(piranha::alignment_check<int>(alignof(int) * 2u));
    CHECK(piranha::alignment_check<std::string>(alignof(std::string) * 4u));
    CHECK(piranha::alignment_check<std::string>(alignof(std::string) * 8u));
    if (alignof(int) > 1u) {
        CHECK(!piranha::alignment_check<int>(alignof(int) / 2u));
    }
    if (alignof(long) > 1u) {
        CHECK(!piranha::alignment_check<long>(alignof(long) / 2u));
    }
    if (alignof(std::string) > 1u) {
        CHECK(!piranha::alignment_check<std::string>(alignof(std::string) / 2u));
    }
#endif
}
...
TEST_CASE("memory_parallel_init_destroy_test")
{
    for (unsigned i = 0u; i <= settings::get_n_threads(); ++i) {
        CHECK_NOTHROW(parallel_value_init(static_cast<int *>(nullptr), 0u, i));
        CHECK_NOTHROW(parallel_destroy(static_cast<int *>(nullptr), 0u, i));
        auto ptr1 = make_parallel_array<int>(small_alloc_size, i);
        CHECK(std::all_of(ptr1.get(), ptr1.get() + small_alloc_size, [](int n) { return n == 0; }));
        auto ptr2 = make_parallel_array<custom_string>(small_alloc_size, i);
        CHECK(std::all_of(ptr2.get(), ptr2.get() + small_alloc_size,
                                [](const std::string &str) { return str == "hello"; }));
        CHECK_THROWS_AS(make_parallel_array<faulty_string>(small_alloc_size, i), std::runtime_error);
        // Check zero sizes.
        CHECK(make_parallel_array<int>(0u, i).get() == nullptr);
        CHECK(make_parallel_array<custom_string>(0u, i).get() == nullptr);
        CHECK(make_parallel_array<faulty_string>(0u, i).get() == nullptr);
        if (sizeof(int) > 1u) {
            CHECK_THROWS_AS(make_parallel_array<int>(std::numeric_limits<std::size_t>::max(), i), std::bad_alloc);
        }
        ...
        CHECK(std::equal(ptr6.get(), ptr6.get() + small_alloc_size, ptr_cmp.get()));
        CHECK(ptr5.get() == nullptr);
        auto ptr7 = make_parallel_array<custom_string>(small_alloc_size, i);
        ptr7 = std::move(ptr6);
        CHECK(ptr6.get() == nullptr);
        CHECK(std::equal(ptr7.get(), ptr7.get() + small_alloc_size, ptr_cmp.get()));
    }
}
