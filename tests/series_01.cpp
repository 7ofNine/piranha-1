...
struct debug_access<construction_tag> {
    ...
    struct runner {
        ...
        void operator()(const Expo &) const
        {
            ...
            // Default constructor.
            CHECK(series_type().empty());
            CHECK(series_type().size() == 0u);
            CHECK(series_type().get_symbol_set().size() == 0u);
            // Copy constructor.
            series_type s;
            s.m_symbol_set = symbol_fset{"x"};
            s.insert(term_type(Cf(1), key_type{Expo(1)}));
            series_type t(s);
            CHECK(*s.m_container.begin() == *t.m_container.begin());
            CHECK(s.m_container.begin()->m_cf == t.m_container.begin()->m_cf);
            CHECK(s.get_symbol_set() == t.get_symbol_set());
            // Move constructor.
            series_type u{series_type(s)};
            CHECK(*u.m_container.begin() == *t.m_container.begin());
            CHECK(u.m_container.begin()->m_cf == t.m_container.begin()->m_cf);
            CHECK(u.get_symbol_set() == t.get_symbol_set());
            auto s2 = s;
            series_type u2(std::move(s2));
            CHECK(s2.empty());
            CHECK(s2.get_symbol_set().size() == 0u);
            // Copy assignment.
            u = t;
            CHECK(*u.m_container.begin() == *t.m_container.begin());
            CHECK(u.m_container.begin()->m_cf == t.m_container.begin()->m_cf);
            ...
            // Move assignment.
            u = std::move(t);
            CHECK(*u.m_container.begin() == *s.m_container.begin());
            CHECK(u.m_container.begin()->m_cf == s.m_container.begin()->m_cf);
            CHECK(u.get_symbol_set() == s.get_symbol_set());
            CHECK(t.empty());
            CHECK(t.get_symbol_set().size() == 0u);
            ...
            CHECK(s1.size() == 1u);
            CHECK(s1.m_container.begin()->m_cf == Cf(1));
            CHECK(s1.m_container.begin()->m_key.size() == 1u);
            CHECK(s1.m_container.begin()->m_key[0u] == Expo(1));
            // Series, different term type, move.
            series_type s1a(std::move(other1));
            CHECK(s1a.size() == 1u);
            CHECK(s1a.m_container.begin()->m_cf == Cf(1));
            CHECK(s1a.m_container.begin()->m_key.size() == 1u);
            CHECK(s1a.m_container.begin()->m_key[0u] == Expo(1));
            ...
            CHECK(so2.size() == 1u);
            CHECK(so2.m_container.begin()->m_cf == Cf(1));
            CHECK(so2.m_container.begin()->m_key.size() == 1u);
            CHECK(so2.m_container.begin()->m_key[0u] == Expo(1));
            // Series, same term type, move.
            series_type so2a(std::move(other2));
            CHECK(so2a.size() == 1u);
            CHECK(so2a.m_container.begin()->m_cf == Cf(1));
            CHECK(so2a.m_container.begin()->m_key.size() == 1u);
            CHECK(so2a.m_container.begin()->m_key[0u] == Expo(1));
            // Construction from non-series.
            series_type s1b(1);
            CHECK(s1b.size() == 1u);
            CHECK(s1b.m_container.begin()->m_cf == Cf(1));
            CHECK(s1b.m_container.begin()->m_key.size() == 0u);
            CHECK(s1b.m_symbol_set.size() == 0u);
            // Construction from coefficient series.
            typedef g_series_type<series_type, Expo> series_type3;
            series_type3 s3o(series_type(5.));
            CHECK(s3o.size() == 1u);
            CHECK(s3o.m_container.begin()->m_cf.size() == series_type(5.).m_container.size());
            series_type3 s4o(series_type("x"));
            CHECK(s4o.m_container.begin()->m_cf.size() == 1u);
            CHECK(s4o.size() == 1u);
            CHECK(s4o.m_container.begin()->m_cf.m_container.begin()->m_cf == 1);
            // Generic assignment.
            // Series, different term type, copy.
            other1 = 0;
            series_type s1c;
            ...
            s1c = other1;
            CHECK(s1c.size() == 1u);
            CHECK(s1c.m_container.begin()->m_cf == Cf(1));
            CHECK(s1c.m_container.begin()->m_key.size() == 1u);
            CHECK(s1c.m_container.begin()->m_key[0u] == Expo(1));
            // Series, different term type, move.
            s1c = std::move(other1);
            CHECK(s1c.size() == 1u);
            CHECK(s1c.m_container.begin()->m_cf == Cf(1));
            ...
            sp2 = other2;
            CHECK(sp2.size() == 1u);
            CHECK(sp2.m_container.begin()->m_cf == Cf(1));
            CHECK(sp2.m_container.begin()->m_key.size() == 1u);
            CHECK(sp2.m_container.begin()->m_key[0u] == Expo(1));
            // Series, same term type, move.
            sp2 = std::move(other2);
            CHECK(sp2.size() == 1u);
            CHECK(sp2.m_container.begin()->m_cf == Cf(1));
            ...
            s1b = 2;
            CHECK(s1b.size() == 1u);
            CHECK(s1b.m_container.begin()->m_cf == Cf(2));
            CHECK(s1b.m_container.begin()->m_key.size() == 0u);
            CHECK(s1b.m_symbol_set.size() == 0u);
            ...
            s5o = series_type("x");
            CHECK(s5o.size() == 1u);
            CHECK(s5o.m_container.begin()->m_cf.size() == 1u);
            CHECK(s5o.m_container.begin()->m_cf.m_container.begin()->m_cf == 1);
            // Type traits.
            CHECK((std::is_constructible<series_type, series_type>::value));
            CHECK((!std::is_constructible<series_type, series_type, int>::value));
// This should be the same problem as in the explicit integer conversion.
#if !defined(PIRANHA_COMPILER_IS_CLANG)
            CHECK((std::is_constructible<series_type2, series_type>::value));
#endif
            CHECK((std::is_constructible<series_type3, series_type>::value));
            CHECK((std::is_assignable<series_type, int>::value));
            CHECK((std::is_assignable<series_type, series_type2>::value));
            CHECK((!std::is_assignable<series_type, symbol_fset>::value));
        }
    };
    template <typename Cf>
...
TEST_CASE("series_constructor_test")
{
...
public:
    ...
    struct runner {
        ...
        void operator()(const Expo &) const
        {
            ...
            s.insert(term_type(Cf(1), key_type{Expo(1)}));
            CHECK(!s.empty());
            CHECK(s.size() == unsigned(1));
            // Insert incompatible term.
            CHECK_THROWS_AS(s.insert(term_type(Cf(1), key_type())), std::invalid_argument);
            CHECK(s.size() == unsigned(1));
...
TEST_CASE("series_insertion_test")
{
    ...
    struct runner {
        ...
        void operator()(const Expo &) const
        {
            ...
            auto check_neg_merge = [&it]() {
                CHECK((it->m_cf == value_type(1) || it->m_cf == value_type(2) || it->m_cf == value_type(3)
            ...
            CHECK((it->m_cf == value_type(1) + value_type(1) || it->m_cf == value_type(2) + value_type(2)
            ...
            CHECK(s1.size() == unsigned(3));
            it = s1.m_container.begin();
            CHECK((it->m_cf == value_type(1) + value_type(1) + value_type(1) + value_type(1)
...
TEST_CASE("series_merge_terms_test")
{
...
public:
    ...
    struct runner {
        ...
        void operator()(const Expo &) const
        {
            ...
            CHECK(merge_out.size() == unsigned(1));
            CHECK(merge_out.m_container.find(term_type(Cf(1), key_type{Expo(0)})) != merge_out.m_container.end());
            auto compat_check = [](const typename series_type::base &series) {
                for (auto it = series.m_container.begin(); it != series.m_container.end(); ++it) {
                    CHECK(it->is_compatible(series.m_symbol_set));
                }
            };
            ...
            CHECK(merge_out.size() == unsigned(3));
            CHECK(merge_out.m_container.find(term_type(Cf(1), key_type{Expo(0), Expo(0)}))
                        != merge_out.m_container.end());
            CHECK(merge_out.m_container.find(term_type(Cf(1), key_type{Expo(1), Expo(0)}))
                        != merge_out.m_container.end());
            CHECK(merge_out.m_container.find(term_type(Cf(2), key_type{Expo(2), Expo(0)}))
                        != merge_out.m_container.end());
            compat_check(merge_out);
        }
    };
...
TEST_CASE("series_merge_arguments_test")
{
