...
TEST_CASE("subs_series_subs_test")
{
    // Substitution on key only.
    using stype0 = g_series_type<rational, monomial<int>>;
    // Type trait checks.
    CHECK((has_subs<stype0, int>::value));
    CHECK((has_subs<stype0, double>::value));
    CHECK((has_subs<stype0, integer>::value));
    CHECK((has_subs<stype0, rational>::value));
#if defined(MPPP_WITH_MPFR)
    CHECK((has_subs<stype0, real>::value));
#endif
    CHECK((!has_subs<stype0, std::string>::value));
    {
        stype0 x{"x"}, y{"y"}, z{"z"};
        ...
        CHECK(tmp == y + 2);
        CHECK(tmp.is_identical(math::subs<int>(x + y, {{"x", 2}})));
        ...
        CHECK(tmp5.is_identical(math::subs<integer>(3 * x + y * y / 7, {{"y", -2_z}})));
        CHECK((std::is_same<decltype(tmp5), stype0>::value));
        CHECK(tmp5 == 3 * x + piranha::pow(-2_z, 2) / 7_q);
        // Substitution with series.
        auto tmp6 = (3 * x + y * y / 7).subs<stype0>({{"y", z * 2}});
        CHECK(tmp6.is_identical(math::subs<stype0>(3 * x + y * y / 7, {{"y", z * 2}})));
    ...
    CHECK((!key_has_subs<stype1::term_type::key_type, rational>::value));
    CHECK((has_subs<stype1, int>::value));
    CHECK((has_subs<stype1, double>::value));
    CHECK((has_subs<stype1, integer>::value));
    CHECK((has_subs<stype1, rational>::value));
#if defined(MPPP_WITH_MPFR)
    CHECK((has_subs<stype1, real>::value));
#endif
    CHECK((!has_subs<stype1, std::string>::value));
    {
        ...
        auto tmp5 = (3 * x + y * y / 7).subs<integer>({{"y", -2_z}});
        CHECK(tmp5.is_identical(math::subs<integer>(3 * x + y * y / 7, {{"y", -2_z}})));
        CHECK((std::is_same<decltype(tmp5), stype1>::value));
        CHECK(tmp5 == 3 * x + piranha::pow(-2_z, 2) / 7_q);
        auto tmp6 = (3 * x + y * y / 7).subs<stype1>({{"y", -2 * z}});
        CHECK(tmp6.is_identical(math::subs<stype1>(3 * x + y * y / 7, {{"y", -2 * z}})));
    ...
    CHECK((key_has_subs<stype2::term_type::key_type, rational>::value));
    CHECK((has_subs<stype2, int>::value));
    CHECK((has_subs<stype2, double>::value));
    CHECK((has_subs<stype2, integer>::value));
    CHECK((has_subs<stype2, rational>::value));
#if defined(MPPP_WITH_MPFR)
    CHECK((has_subs<stype2, real>::value));
#endif
    CHECK((!has_subs<stype2, std::string>::value));
    {
        ...
        CHECK(tmp == (2 + y) * z);
        CHECK(tmp.is_identical(math::subs<int>((x + y) * z, {{"x", 2}})));
        ...
        CHECK(tmp2 == (2. + y) * z);
        CHECK(tmp2.is_identical(math::subs<double>((x + y) * z, {{"x", 2.}})));
        CHECK(
        ...
        CHECK(tmp3.is_identical(math::subs<rational>((3 * x + y * y / 7) * z, {{"z", 2 / 5_q}})));
        CHECK((std::is_same<decltype(tmp3), stype2>::value));
        CHECK(tmp3 == (3 * x + y * y / 7) * 2 / 5_q);
        auto tmp4 = ((3 * x + y * y / 7) * z).subs<rational>({{"y", 2 / 3_q}, {"z", 4_q}});
        CHECK(tmp4.is_identical(math::subs<rational>((3 * x + y * y / 7) * z, {{"y", 2 / 3_q}, {"z", 4_q}})));
        CHECK((std::is_same<decltype(tmp4), stype2>::value));
        CHECK(tmp4 == (3 * x + 2 / 3_q * 2 / 3_q / 7) * 4_z);
#if defined(MPPP_WITH_MPFR)
        auto tmp5 = ((3 * x + y * y / 7) * z).subs<real>({{"y", -2.123_r}});
        CHECK(tmp5.is_identical(math::subs<real>((3 * x + y * y / 7) * z, {{"y", -2.123_r}})));
        CHECK(
            (std::is_same<decltype(tmp5), g_series_type<g_series_type<real, monomial<int>>, monomial<int>>>::value));
        CHECK(tmp5 == (3 * x + piranha::pow(-2.123_r, 2) / 7) * z);
#endif
        auto tmp6 = ((3 * x + y * y / 7) * z).subs<stype2>({{"z", 2 * t}});
        CHECK(tmp6.is_identical(math::subs<stype2>((3 * x + y * y / 7) * z, {{"z", 2 * t}})));
    ...
    }
    {
        ...
        CHECK((x1 * x2 * y * 4 / 3_q + 2 * y).subs<int>({{"x", 4}}) == 16 * y * 4 / 3_q + 2 * y);
        CHECK((x1 * x2 * y * 4 / 3_q + 2 * y).subs<int>({{"x", 4},{"y", 5}}) == 16 * 5 * 4 / 3_q + 2 * 5);
    }
}

#if defined(PIRANHA_WITH_BOOST_S11N)

TEST_CASE("subs_series_serialization_test")
{
